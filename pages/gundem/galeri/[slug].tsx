...
export async function getStaticPaths() {
    ...
    return {
        paths: Array.from(res).map((item: any) => `/gundem/galeri/${item.attributes.slug}_${item.id}`) || [],
        fallback: false,
    }
}
...
export async function getStaticProps({params}: any) {
    ...
    try {
        res = await getSingularNews(params.slug.split("_")[1], "gallery-details").then(data => data.data.data)
    } catch (e: any) {
        console.log(e)
    }
...
const GundemGalleryDetailPage: NextPage = ({data}: any) => {
    const {category_name, updatedAt, createdAt, gallery_item_contents, news_spot_text, news_title} = data.attributes;
                            ...
                            {Array.from(gallery_item_contents).map((item: any, index) => (
                                <div className={style.newsCard} key={index}>
                                    <Image
                                        ...
                                        layout={"responsive"}
                                    />
                                    <span className={style.counter}>{index + 1} / {gallery_item_contents.length}</span>
                                    <div className={style.galleryBodyText} dangerouslySetInnerHTML={{__html: item?.gallery_text_content}}/>
                                </div>
                            ))}
                        </div>
