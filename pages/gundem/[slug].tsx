...
export async function getStaticPaths() {
    ...
    try {
        res = getNewsDetail("gündem").then(data => data.data.data);
    ...
    return {
        paths: Array.from(res).map((item: any) => `/gundem/${item.attributes.slug}_${item.id}`) || undefined,
        fallback: true,
    }
}
...
export async function getStaticProps({params}: any) {
    ...
    try {
        res = await getSingularNews(params.slug.split("_")[1], "news-details").then(data => data.data.data)
    } catch (e: any) {
        console.log(e)
    }
